	...
	public class PlayerHandlers
	{
		...
		public void OnPlayerHurt(HurtingEventArgs ev)
		{
			if (ev.Target != ev.Attacker)
			{
				...
				if (plugin.Config.Scp0492Damage != 40 && ev.Attacker.Role == RoleType.Scp0492) ev.Amount = plugin.Config.Scp0492Damage;
				else if (plugin.Config.Scp106Damage != 40 && ev.Attacker.Role == RoleType.Scp106) ev.Amount = plugin.Config.Scp106Damage;
				else if (plugin.Config.Scp939Damage != 65 && ev.Attacker.Role.Is939()) ev.Amount = plugin.Config.Scp939Damage;
			}
		}
